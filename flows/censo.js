...
const normalizarTexto = texto => texto.normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toLowerCase();
...
module.exports = async function flujoCenso(incomingMsg, from, estadoConversacion) {
  ...
  if (!estado || !estado.paso.startsWith('censo')) return null;
  ...
  switch (estado.paso) {
    ...
    case 'censo_esperando_tipo_filtro': {
      if (msg === 'localidad') {
        ...
        return 'Por favor respondé *localidad* o *vendedor*';
      }
    }
    ...
    case 'censo_esperando_tipo_vendedor': {
      ...
      if (tipo !== 'crecemos' && tipo !== 'caemos') {
        return 'Por favor respondé *crecemos* o *caemos*';
      }
...
respuestaFinal = `📊 Top 5 puntos donde ${tipo} en "${valorFiltro}" (Canal: ${canalFiltro}):\n\n` +
  ...
  ↪ SHARE CMQ: ${p.share}
  ↪ SHARE CMQ sep24: ${p.sharesep24}
  ↪ SHARE CMQ abril24: ${p.shareabril24}`
  ).join('\n\n');
